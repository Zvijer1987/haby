...
function buildOverallExpectedSeries(habits: Habit[], histories: Record<number, { date: string; value: number }[]>, days: number) {
...
export default function RightPanel({ widgets, habits, histories, selectedCategory, onAddWidget, onDeleteWidget, onReorderWidgets, onUpdateWidget }: {
  widgets: Widget[];
  habits: Habit[];
  ...
  const filteredHabits = useMemo(() => habits.filter((habit) => selectedCategory === 'all' || String(habit.categoryId) === selectedCategory), [habits, selectedCategory]);

  const selectedDoneItems = useMemo(() => filteredHabits.flatMap((habit) => {
  ...
  const aggregatedHistory = useMemo(() => aggregateHistory(filteredHabits.flatMap((habit) => histories[habit.id] || [])), [filteredHabits, histories]);
  ...
  const widgetCatalog = [
    { key: 'history', label: 'Overall History', add: () => onAddWidget('history', 'Overall History', { type: 'column', days: 14 }), remove: () => removeFirst((widget) => widget.type === 'history') },
    { key: 'calendar', label: 'Calendar', add: () => onAddWidget('calendar', 'Calendar'), remove: () => removeFirst((widget) => widget.type === 'calendar') },
    { key: 'doneList', label: 'Today List', add: () => onAddWidget('doneList', 'Today List'), remove: () => removeFirst((widget) => widget.type === 'doneList') },
  ...
  return (
    <div className="panel stack-gap right-panel" id="customize-widgets-panel">
      <details className="panel-details widget-accordion">
        <summary>Customize Widgets</summary>
        <div className="inner-pad stack-gap widget-catalog-list">
          {widgetCatalog.map((item) => {
            const hasWidget = visibleWidgets.some((widget) => item.key === `habitChart-${widget.config?.habitId}` || widget.type === item.key);
            return (
              <div key={item.key} className="list-row widget-catalog-row">
                <span>{toTitleCase(item.label)}</span>
                <div className="inline-actions">
                  <button type="button" className="soft-btn small-btn" onClick={item.add}>Add</button>
                  <button type="button" className="danger-btn small-btn" onClick={item.remove} disabled={!hasWidget}>Remove</button>
                </div>
              </div>
            );
          })}
      ...
      {visibleWidgets.map((widget) => {
        ...
        const habit = widget.type === 'habitChart' ? filteredHabits.find((item) => item.id === widget.config?.habitId) || habits.find((item) => item.id === widget.config?.habitId) : undefined;
        ...
        const chartSeries = widget.type === 'history'
          ? { ...buildSeries(aggregatedHistory, overallWindowDays, 1), expectedValues: buildOverallExpectedSeries(filteredHabits, histories, overallWindowDays) }
        ...
        return (
          <div key={widget.id} className="widget-shell">
            <div className="widget-config-row widget-header-row">
              ...
              <div className="inline-actions">
                <button type="button" className="ghost-btn small-btn" onClick={() => move(widget.id, -1)}>↑</button>
                <button type="button" className="ghost-btn small-btn" onClick={() => move(widget.id, 1)}>↓</button>
              </div>
            </div>
            ...
            {widget.type === 'history' ? <OverallChart title="Overall History" values={chartSeries.values} labels={chartSeries.labels} expectedValues={chartSeries.expectedValues} type={widget.config?.type === 'column' ? 'column' : 'line'} showTitle={false} pointCount={overallPointCount} /> : null}
              ...
              <div className="widget-card done-widget">
                ...
                {selectedDoneItems.length
                  ? selectedDoneItems.map((item) => (
                      <div key={item.id} className="done-row"><span>{item.icon}</span><span>{toTitleCase(item.name)}</span><strong>{item.value}</strong></div>
                    ))
            ...
            <div className="widget-footer-actions">
              <button type="button" className="danger-btn small-btn" onClick={() => onDeleteWidget(widget.id)}>Remove</button>
            </div>
          </div>
        );
      })}
